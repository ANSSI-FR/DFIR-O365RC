function Get-AADDevices {
    ...
    if ($null -ne $deviceEvents){$deviceEvents = $deviceEvents.ToJsonString() | ConvertFrom-Json}

    # Get all devices
    "Getting all devices" | Write-Log -LogPath $logFile
    $allDevices = Get-MgDevice -All -ErrorAction Stop
    if ($null -ne $allDevices){$allDevices = $allDevices.ToJsonString() | ConvertFrom-Json}
    ...
    if ($null -ne $deletedDevices){$deletedDevices = $deletedDevices.ToJsonString() | ConvertFrom-Json}
    ...
    foreach ($uniqueDevice in $uniqueDevices){
        ...
        if ($deviceObject){
            ...
            if ($null -ne $deviceOwners){$deviceOwners = $deviceOwners.ToJsonString() | ConvertFrom-Json}
            ...
            if ($null -ne $deviceUsers){$deviceUsers = $deviceUsers.ToJsonString() | ConvertFrom-Json}
